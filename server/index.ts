...
import { setupWatcherLogRotation } from "./rotateWatchersLog";

function getArchiveLogPath(watchersLogPath: string): string {
  const parts = watchersLogPath.split(".");
...
async function main() {
  ...
  const cameras = config.cameras.map(
    (camera: any) =>
      new Camera({
  ...
  const archiveWatchersLog = new TextFileWatcherLog(
    getArchiveLogPath(config.watcherLog.filename)
  );
  const watcherCounter = new WatcherCounter(watcherLog, cameras, backends);
  if (config.watcherLog.enable) {
    setupWatcherLogRotation(
      watcherLog,
      archiveWatchersLog,
      config.watcherLog.retentionDays
    );
  ...
  scheduler.on("update", (camera: Camera) =>
    console.log("Update:", camera.name, camera.id)
  );
  scheduler.on(
    "updateAttemptError",
    (error: string, camera: Camera, attempt: number) => {
