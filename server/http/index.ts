...
import { watchersLogView } from "./watchersLog";
...
export function setupExpressApp(
  ...
  const jwtAuth = jwtAuthMiddleware(
    authSettings.jwtSignKey,
    authorizationHeaderTokenExtractor
  );
  const cookieAuth = jwtAuthMiddleware(
    authSettings.jwtSignKey,
    cookieTokenExtractor,
    "/"
  );
  ...
  app.post(
    ...
    login(
      ...
      authSettings.jwtSignKey,
      authSettings.jwtLifetime
    )
  );
  app.get(
    "/cameras",
    jwtAuth,
    getCameraList(cameras, cameraStateStorage, watcherCounter, backends)
  );
  app.get(
    "/watchers-log",
    cookieParser(),
    cookieAuth,
    watchersLogView(watcherLog, watchersLogShowDays)
  );
  app.patch(
    "/cameras/:id",
    jwtAuth,
    bodyParser.json(),
    updateCamera(cameras, cameraStateStorage, websocketServer)
  );
